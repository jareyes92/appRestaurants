...
export default function InfoUser(props) {
  ...
  const uploadImage = async (uri, nameImage) => {
    ...
    setIsLoading(true);
    const response = await fetch(uri); // obtengo la imagen
  ...
  const updatePhotoUrl = uid => {
    firebase
      ...
      .then(async result => {
        ...
        await firebase.auth().currentUser.updateProfile(update); //actalizando la foto de perfil
        setReloadData(true);
        setIsLoading(false);
      })
      .catch(() => {
    ...
    <View style={styles.viewUserInfo}>
      <Avatar
        ...
        source={{
          uri: photoURL
            ? photoURL
            : "https://api.adorable.io/avatars/219/abott@adorable.png"
        }}
      />
      <View>
